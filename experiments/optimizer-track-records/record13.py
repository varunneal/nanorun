...
Muon update is replaced by NorMuonH — Muon's NS-orthogonalised direction with an additional
Adafactor-style variance preconditioning along the short axis (a la NorMuon, see
https://arxiv.org/pdf/2510.05491), applied via a hyperball projection that preserves the
Frobenius norm of every hidden 2D weight matrix at every step. The residual-side projections
/ mlp.fc are initialised with per-module multipliers on the default Kaiming init so NorMuonH
has non-zero matrices to operate on from step 0.
"""
...
########################################
#       MuLoCo Outer Optimizer         #
########################################

class NesterovOuterOptimizer:
    """MuLoCo-style outer Nesterov SGD wrapper around the inner optimizer(s).
    Every `sync_interval` inner steps, computes the pseudogradient (snapshot - current_params)
    and applies an outer Nesterov SGD update. Algorithm 1 (K=1) of the MuLoCo paper.

    Note: NorMuonH already shards + all_gathers params across ranks (see line ~265), so all
    ranks hold identical p.data after each inner step. No extra sync needed here."""

    def __init__(self, model_params, outer_lr=0.5, outer_momentum=0.5, sync_interval=30):
    ...
    def _outer_step(self):
        ...
        for i, p in enumerate(self._all_params):
            ...
            delta = snapshot - p.data                              # pseudogradient
...
# MuLoCo: configurable via TRAIN_STEPS env var; default 3240 = 108 * H=30 clean outer steps.
train_steps = int(os.environ.get("TRAIN_STEPS", 3240))

# MuLoCo outer optimizer hyperparameters (env-driven for sweep).
USE_OUTER_OPTIMIZER = os.environ.get("USE_OUTER_OPTIMIZER", "1") == "1"
OUTER_LR = float(os.environ.get("OUTER_LR", 0.5))
...
optimizer2 = NorMuonH(qkv_params, lr=0.018)
optimizer3 = NorMuonH(mlp_fc_params, lr=0.018)
optimizer4 = NorMuonH(attn_proj_params, lr=0.018)
optimizer5 = NorMuonH(mlp_proj_params, lr=0.018)
optimizers = [optimizer1, optimizer2, optimizer3, optimizer4, optimizer5]

# MuLoCo outer Nesterov SGD wrapper (Algorithm 1, K=1). Wraps ALL trainable params.
outer_optimizer = None
if USE_OUTER_OPTIMIZER:
    outer_optimizer = NesterovOuterOptimizer(
        list(model.parameters()),
        outer_lr=OUTER_LR,
        outer_momentum=OUTER_MOMENTUM,
        sync_interval=SYNC_INTERVAL,
    )
    print0(f"MuLoCo outer optimizer ENABLED: olr={OUTER_LR} omom={OUTER_MOMENTUM} "
           f"H={SYNC_INTERVAL} train_steps={train_steps}", console=True)
else:
...
for step in range(train_steps + 1):
    ...
    # MuLoCo outer Nesterov step (fires every SYNC_INTERVAL inner steps).
    if outer_optimizer is not None:
        outer_optimizer.maybe_outer_step()
