...
SCHEDULE_STEPS = int(os.environ.get("SCHEDULE_STEPS", str(TRAIN_STEPS)))
LR_POWER = float(os.environ.get("LR_POWER", "1.0"))
LR_FLOOR = float(os.environ.get("LR_FLOOR", "0.0"))
ADAM_LR_FLOOR = float(os.environ.get("ADAM_LR_FLOOR", str(LR_FLOOR)))
KLSOAP_LR_FLOOR = float(os.environ.get("KLSOAP_LR_FLOOR", str(LR_FLOOR)))
ADAM_COOLDOWN_FRAC = float(os.environ.get("ADAM_COOLDOWN_FRAC", "0.4"))
KLSOAP_COOLDOWN_FRAC = float(os.environ.get("KLSOAP_COOLDOWN_FRAC", "1.0"))
KLSOAP_LR = float(os.environ.get("KLSOAP_LR", "0.018"))
KLSOAP_BETA1 = float(os.environ.get("KLSOAP_BETA1", "0.95"))
KLSOAP_BETA2 = float(os.environ.get("KLSOAP_BETA2", "0.9"))
KLSOAP_SHAMPOO_BETA = float(os.environ.get("KLSOAP_SHAMPOO_BETA", "0.95"))
KLSOAP_PRECONDITION_FREQUENCY = int(os.environ.get("KLSOAP_PRECONDITION_FREQUENCY", "1"))
TRAIN_PROGRESS_INTERVAL = int(os.environ.get("TRAIN_PROGRESS_INTERVAL", "0"))
EXTRA_VAL_STEPS_ENV = os.environ.get("EXTRA_VAL_STEPS", "")
if SEED_ENV:
...
class KLSOAPH(torch.optim.Optimizer):
    def __init__(
        ...
        shampoo_beta=0.95, eps=1e-8, precondition_frequency=1,
    ):
...
if dist.get_rank() == 0:
    ...
    logfile = f"{log_dir}/{uuid.uuid4()}.txt"
    print(logfile)
...
print0(f"Using seed_env={SEED_ENV}")
print0(f"Using train_steps={TRAIN_STEPS}")
print0(f"Using schedule_steps={SCHEDULE_STEPS}")
print0(f"Using lr_power={LR_POWER}")
print0(f"Using lr_floor={LR_FLOOR}")
print0(f"Using adam_lr_floor={ADAM_LR_FLOOR}")
print0(f"Using klsoap_lr_floor={KLSOAP_LR_FLOOR}")
print0(f"Using adam_cooldown_frac={ADAM_COOLDOWN_FRAC}")
print0(f"Using klsoap_cooldown_frac={KLSOAP_COOLDOWN_FRAC}")
print0(f"Using klsoap_lr={KLSOAP_LR}")
print0(f"Using klsoap_beta1={KLSOAP_BETA1}")
print0(f"Using klsoap_beta2={KLSOAP_BETA2}")
print0(f"Using klsoap_shampoo_beta={KLSOAP_SHAMPOO_BETA}")
print0(f"Using klsoap_precondition_frequency={KLSOAP_PRECONDITION_FREQUENCY}")
print0(f"Using train_progress_interval={TRAIN_PROGRESS_INTERVAL}")
print0(f"Using extra_val_steps_env={EXTRA_VAL_STEPS_ENV}")
print0("="*100)
...
for _ in range(num_trials):
    ...
    extra_val_steps = {train_steps}
    if EXTRA_VAL_STEPS_ENV:
        extra_val_steps |= {int(x) for x in EXTRA_VAL_STEPS_ENV.replace(",", " ").split()}
    print0(f"Using extra_val_steps={sorted(extra_val_steps)}")
    ...
    optimizer2 = KLSOAPH(qkv_params, lr=KLSOAP_LR, beta1=KLSOAP_BETA1, beta2=KLSOAP_BETA2,
                         shampoo_beta=KLSOAP_SHAMPOO_BETA,
                         precondition_frequency=KLSOAP_PRECONDITION_FREQUENCY)
    optimizer3 = KLSOAPH(mlp_fc_params, lr=KLSOAP_LR, beta1=KLSOAP_BETA1, beta2=KLSOAP_BETA2,
                         shampoo_beta=KLSOAP_SHAMPOO_BETA,
                         precondition_frequency=KLSOAP_PRECONDITION_FREQUENCY)
    optimizer4 = KLSOAPH(attn_proj_params, lr=KLSOAP_LR, beta1=KLSOAP_BETA1, beta2=KLSOAP_BETA2,
                         shampoo_beta=KLSOAP_SHAMPOO_BETA,
                         precondition_frequency=KLSOAP_PRECONDITION_FREQUENCY)
    optimizer5 = KLSOAPH(mlp_proj_params, lr=KLSOAP_LR, beta1=KLSOAP_BETA1, beta2=KLSOAP_BETA2,
                         shampoo_beta=KLSOAP_SHAMPOO_BETA,
                         precondition_frequency=KLSOAP_PRECONDITION_FREQUENCY)
    ...
    for opt in (optimizer2, optimizer3, optimizer4, optimizer5):
        for group in opt.param_groups:
            group["cooldown_frac"] = KLSOAP_COOLDOWN_FRAC
            group["lr_floor"] = KLSOAP_LR_FLOOR
    for group in optimizer1.param_groups:
        group["cooldown_frac"] = ADAM_COOLDOWN_FRAC
        group["lr_floor"] = ADAM_LR_FLOOR

    def _eta(progress, cooldown_frac, power=1.0, floor=0.0):
        if progress < 1 - cooldown_frac:
            return 1.0
        x = max(0.0, (1 - progress) / cooldown_frac)
        return floor + (1 - floor) * (x ** power)
    ...
    def set_hparams(step):
        ...
        for opt in optimizers:
            for group in opt.param_groups:
                eta = _eta(progress, group["cooldown_frac"], LR_POWER, group["lr_floor"])
    ...
    for step in range(train_steps + 1):
        ...
        if step == train_steps or step % val_step_freq == 0 or step in extra_val_steps:
            # stop the clock
            dist.barrier()
